...
var NeuralNetwork = function(options) {
  ...
  this.activationFn = options.activationFn || 'logisticSigmoid';
}
...
NeuralNetwork.prototype = {
  ...
  runInput: function(input) {
    ...
    for (var layer = 1; layer <= this.outputLayer; layer++) {
      for (var node = 0; node < this.sizes[layer]; node++) {
        ...
        this.outputs[layer][node] = activationFuncs[this.activationFn](sum);
      }
  ...
  fromJSON: function(json) {
   ...
   toFunction: function() {
    var funcs = [exp, activationFuncs[this.activationFn]].map(function (f) {return f.toString()}).join('\n');
...
'  var net = ' + JSON.stringify(json) + ';\n ' + funcs + '\n\n\
  for (var i = 1; i < net.layers.length; i++) {\n\
    ...
    for (var id in layer) {\n\
      ...
      output[id] = '+this.activationFn+'(sum);\n\
    }\n\
    input = output;\n\
  }\n\
  return output;');
